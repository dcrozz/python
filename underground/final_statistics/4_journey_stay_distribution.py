...
if __name__ == '__main__':
    ...
    for file_name in file_names:
        ...
        with open(file_name) as data:
            ...
            for num in range(i):
                try:
                    cur_line = lines[num].strip().split(',')
                    next_line = lines[num+1].strip().split(',')
                    time_delta = time_diff(cur_line[4],next_line[3])
                    cur_date = cur_line[-1]
                    next_date = next_line[-1]
                    if cur_date != next_date:
                        #calculate the date delta
                        #if cur_date[:4]=='2011' and next_date[:4]=='2012':
                            #date_delta = (int(next_date[-2:]) + (month.get(cur_date[-4:-2]) -int(cur_date[-2:])))*86400
                        if cur_date[-4:-2] != next_date[-4:-2]:
                            date_delta = (int(next_date[-2:]) + (month.get(cur_date[-4:-2]) -int(cur_date[-2:])))*86400
                        else:
                            date_delta = (int(next_date[-2:]) - int(cur_date[-2:]))*86400
                        time_delta = time_delta + date_delta
                    cur_line.append(str(time_delta))
                    if cur_line[2] == top1:
                        with open(file_name[:-4] +'top1stay.txt','a') as out:
                            out.write(','.join(cur_line)+'\n')
                    elif cur_line[2] == top2:
                        with open(file_name[:-4] +'top2stay.txt','a') as out:
                            out.write(','.join(cur_line)+'\n')
                    else:
                        with open(file_name[:-4] +'otherstay.txt','a') as out:
                            out.write(','.join(cur_line)+'\n')
                except Exception,e:
                    print 'out of range'
            #change line
